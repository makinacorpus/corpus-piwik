  ...
  file.managed:
    ...
    - contents: |
            ...
            if [ -e "{{cfg.project_root}}" ];then
             ...
             find "{{cfg.project_root}}" -maxdepth 1 -mindepth 1|egrep -v "/(plugins|bin|lib|www|\.git)"|while read f;do
               {{locs.resetperms}} -q\
                  --fmode 661 --dmode 551 \
                  -u {{cfg.user}} -g {{cfg.group}}\
                  --paths "$f"\
             ...
             find "{{cfg.data_root}}/var" "{{cfg.project_root}}" -maxdepth 1 -mindepth 1|\
              egrep "/(plugins|bin|lib|www|sites|run|private|tmp|log)"|while read f;do
                {{locs.resetperms}} -q\
                  --fmode 771 --dmode 771 \
                  -u {{cfg.user}} -g {{cfg.group}}\
                  --paths "$f"\
             ...
             {{locs.resetperms}} -q --no-recursive\
               ...
               --groups {{cfg.group}}:rwx \
               --groups {{salt['mc_apache.settings']().httpd_user}}:r-x;
             #setfacl -b -k -R "{{cfg.project_root}}/plugins" -"{{cfg.project_root}}/www"
             find "{{cfg.project_root}}/plugins" "{{cfg.project_root}}/{{cfg.data.download_name}}" -mindepth 0 -maxdepth 0|while read f;do
               {{locs.resetperms}} -q\
                --fmode 770 --dmode 770 --paths "${f}"\
                -u {{cfg.user}} -g {{cfg.group}}\
                --groups {{salt['mc_apache.settings']().httpd_user}}:r-x\
                --users {{cfg.user}}:rwx --groups {{cfg.group}}:rwx;
             done
